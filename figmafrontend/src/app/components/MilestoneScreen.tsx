...
export function MilestoneScreen({ level, onContinue, onCashOut }: MilestoneScreenProps) {
  ...
  const milestone = milestones[level as keyof typeof milestones];
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 pb-24 relative overflow-hidden">
      ...
      <div className="relative z-10 text-center">
        ...
        <p className="text-gray-400 mb-8">
          You've secured a minimum prize
        </p>
            ...
            {[5, 10, 15].map(lvl => (
              <div
                ...
                className={`w-12 h-12 rounded-full flex items-center justify-center font-bold transition-all ${
                  lvl === level
                    ? 'bg-gradient-to-br from-[#FFD700] to-[#FFA500] scale-110'
                    : lvl < level
        ...
        <div className="space-y-3 max-w-sm mx-auto">
          ...
          {level === 15 && (
            <button
              onClick={onContinue}
              className="w-full bg-gradient-to-r from-[#FFD700] to-[#FFA500] text-black font-bold py-4 rounded-2xl hover:scale-[1.02] active:scale-[0.98] transition-transform shadow-lg shadow-[#FFD70050]"
            >
              Claim Victory! 🎉
            </button>
          )}
        ...
        {level < 15 && (
          <p className="mt-6 text-xs text-gray-500 max-w-xs mx-auto">
            ⚠️ Continue at your own risk. Wrong answer means you drop back to ${milestone.amount.toLocaleString()}
          </p>
        )}
      </div>
